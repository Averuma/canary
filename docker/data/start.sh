...
CANARY_RATE_ATTACK_SPEED="${CANARY_RATE_ATTACK_SPEED:-1.0}"
...
if [[ ! "$CANARY_RATE_ATTACK_SPEED" =~ ^[0-9]+([.][0-9]+)?$ ]] || [ "$CANARY_RATE_ATTACK_SPEED" = "0" ] || [[ "$CANARY_RATE_ATTACK_SPEED" =~ ^0[.]0+$ ]]; then
	echo "Invalid CANARY_RATE_ATTACK_SPEED: '${CANARY_RATE_ATTACK_SPEED}'. Use a positive number." >&2
	exit 1
fi
...
echo "CANARY_RATE_ATTACK_SPEED:[$CANARY_RATE_ATTACK_SPEED]"
...
set_lua_number "rateAttackSpeed" "$CANARY_RATE_ATTACK_SPEED"
